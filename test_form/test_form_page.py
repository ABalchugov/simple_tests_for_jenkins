...
def test_positive_form_submission(
    ...
    with allure.step("Проверить введенное имя"):
        assert output["name"] == name.strip()

    with allure.step("Проверить введенный email"):
        assert output["email"] == email.strip()

    with allure.step("Проверить текущий адрес"):
        assert output["current_address"] == current_address.strip()

    with allure.step("Проверить постоянный адрес"):
...
def test_partial_form_submission(
        driver,
        name,
        email,
        current_address,
    ...
    with allure.step("Проверить отображение блока результатов"):
        assert output is not None, (
            "Форма должна отправляться при частичном заполнении"
        )

    with allure.step("Проверить заполненные поля"):
        if name:
        ...
        if current_address:
            assert output["current_address"] == current_address

        if permanent_address:
            assert output["permanent_address"] == permanent_address
    ...
    [
        ...
        pytest.param(
            ...
            marks=pytest.mark.xfail(
                reason="Ожидаемая ошибка: нет доменной зоны верхнего уровня"
            ),
        ),
        pytest.param(
            "john@missing-dot",
            marks=pytest.mark.xfail(
                reason="Ожидаемая ошибка: нет точки в домене"
            ),
        ),
        "john@@example.com",
        "john@example..com",
...
def test_invalid_email_validation(driver, invalid_email):
    ...
    with allure.step("Проверить валидацию введенного email"):
        assert (
                output is None or form_po.is_email_error_present()
        ), f"Email '{invalid_email}' не должен быть принят системой"
...
def test_empty_form_submission(driver):
    ...
    with allure.step("Проверить результат отправки пустой формы"):
